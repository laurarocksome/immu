"use client"

export const dynamic = 'force-dynamic'

export default function Home() {
  return (
    <div style={{
      minHeight: '100vh',
      background: 'linear-gradient(135deg, #e8e4f3 0%, #f0eaf9 50%, #f8f6ff 100%)',
      display: 'flex',
      alignItems: 'center',
      justifyContent: 'center',
      padding: '20px',
      fontFamily: 'system-ui, -apple-system, sans-serif'
    }}>
      <div style={{
        maxWidth: '400px',
        width: '100%',
        textAlign: 'center'
      }}>
        {/* Logo */}
        <div style={{ marginBottom: '40px' }}>
          <svg width="140" height="30" viewBox="0 0 362 76.01" xmlns="http://www.w3.org/2000/svg" style={{ margin: '0 auto' }}>
            <rect fill="#2c3060" x="115.67" y="18.41" width="11.13" height="39.19" />
            <path fill="#2c3060" d="M362,18.41v20.94c0,10.4-8.47,18.86-18.87,18.86h-22.26c-10.4,0-18.87-8.46-18.87-18.86v-20.94h11.13v20.94c0,4.27,3.47,7.73,7.73,7.73h7.68s5.52,0,5.52,0h9.06c4.26,0,7.73-3.46,7.73-7.72v-20.94h11.13Z" />
            <path fill="#2c3060" d="M210.36,57.6h-11.13v-26.58c0-1.07-.68-1.61-1.08-1.84-.4-.22-1.21-.51-2.13.05,0,0-13.29,8.2-13.55,8.36-5.93,3.64-13.4,3.64-19.33,0-.26-.16-13.54-8.35-13.54-8.35-.93-.57-1.74-.28-2.14-.06-.4.22-1.08.76-1.08,1.84v26.58h-11.13v-26.58c0-4.87,2.54-9.19,6.8-11.56,4.26-2.37,9.27-2.26,13.41.31,0,0,13.23,8.16,13.48,8.31,2.38,1.46,5.38,1.46,7.76,0,.25-.15,13.49-8.32,13.49-8.32,4.12-2.56,9.14-2.68,13.4-.3,4.25,2.37,6.8,6.69,6.8,11.56v26.58Z" />
            <path fill="#2c3060" d="M293.92,57.6h-11.13v-26.58c0-1.07-.68-1.61-1.08-1.84-.4-.22-1.21-.51-2.13.05,0,0-13.29,8.2-13.55,8.36-5.93,3.64-13.4,3.64-19.33,0-.26-.16-13.54-8.35-13.54-8.35-.93-.57-1.74-.28-2.14-.06-.4.22-1.08.76-1.08,1.84v26.58h-11.13v-26.58c0-4.87,2.54-9.19,6.8-11.56,4.26-2.37,9.27-2.26,13.41.31,0,0,13.23,8.16,13.48,8.31,2.38,1.46,5.38,1.46,7.76,0,.25-.15,13.49-8.32,13.49-8.32,4.12-2.56,9.14-2.68,13.4-.3,4.25,2.37,6.8,6.69,6.8,11.56v26.58Z" />
            <path fill="#da83d2" d="M0,50.71c0-1.26.35-2.51,1.05-3.64l4.33-7.03c.78-1.26.78-2.83,0-4.09l-4.34-7.03c-1.34-2.16-1.4-4.78-.16-7.01,1.24-2.22,3.5-3.55,6.04-3.55h6.75v5.82h-6.75c-.56,0-.84.35-.96.56-.12.21-.27.64.03,1.11l4.34,7.04c1.94,3.14,1.94,7.05,0,10.2l-4.34,7.03c-.3.48-.15.91-.03,1.12.12.21.4.57.96.57h11.7c3.07,0,5.56,2.49,5.56,5.56v.26H6.93c-2.55,0-4.81-1.33-6.05-3.55-.59-1.06-.89-2.22-.89-3.37" />
            <path fill="#da83d2" d="M76.01,25.29c0,1.26-.35,2.51-1.05,3.64l-4.33,7.03c-.78,1.26-.78,2.83,0,4.09l4.34,7.03c1.34,2.16,1.4,4.78.16,7.01-1.24,2.22-3.5,3.55-6.04,3.55h-6.75v-5.82h6.75c.56,0,.84-.35.96-.56.12-.21.27-.64-.03-1.11l-4.34-7.04c-1.94-3.14-1.94-7.05,0-10.2l4.34-7.03c.3-.48.15-.91.03-1.12-.12-.21-.4-.57-.96-.57h-11.43c-3.21,0-5.82-2.61-5.82-5.82h17.25c2.55,0,4.81,1.33,6.05,3.55.59,1.06.89,2.22.89,3.37" />
            <path fill="#da83d2" d="M50.71,76.01c-1.26,0-2.51-.35-3.64-1.05l-7.03-4.33c-1.26-.78-2.83-.78-4.09,0l-7.03,4.34c-2.16,1.34-4.78,1.4-7.01.16-2.22-1.24-3.55-3.5-3.55-6.04v-6.75h5.82v6.75c0,.56.35.84.56.96.21.12.64.27,1.11-.03l7.04-4.34c3.14-1.94,7.05-1.94,10.2,0l7.03,4.34c.48.3.91.15,1.12.03.21-.12.57-.4.57-.96v-11.43c0-3.21,2.61-5.82,5.82-5.82v17.25c0,2.55-1.33,4.81-3.55,6.05-1.06.59-2.22.89-3.37.89" />
            <path fill="#da83d2" d="M25.3,0c1.26,0,2.51.35,3.64,1.05l7.03,4.33c1.26.78,2.83.78,4.09,0l7.03-4.34c2.16-1.34,4.78-1.4,7.01-.16,2.22,1.24,3.55,3.5,3.55,6.04v6.75h-5.82v-6.75c0-.56-.35-.84-.56-.96-.21-.12-.64-.27-1.11.03l-7.04,4.34c-3.14,1.94-7.05,1.94-10.2,0l-7.03-4.34c-.48-.3-.91-.15-1.12-.03-.21.12-.57.4-.57.96v11.43c0,3.21-2.61,5.82-5.82,5.82V6.93c0-2.55,1.33-4.81,3.55-6.05,1.06-.59,2.22-.89,3.37-.89" />
          </svg>
        </div>

        <h1 style={{
          fontSize: '20px',
          color: '#6e6d8f',
          marginBottom: '32px',
          lineHeight: '1.5'
        }}>
          Immu Health, your personal guide to the Autoimmune Protocol diet
        </h1>

        <div style={{
          backgroundColor: 'rgba(255, 255, 255, 0.95)',
          backdropFilter: 'blur(10px)',
          borderRadius: '16px',
          padding: '32px',
          boxShadow: '0 4px 6px rgba(0, 0, 0, 0.1)',
          border: '1px solid rgba(255, 255, 255, 0.3)'
        }}>
          <a
            href="/login"
            style={{
              display: 'block',
              background: 'linear-gradient(to right, #da83d2, #c45fbc)',
              color: 'white',
              padding: '14px 24px',
              borderRadius: '12px',
              textDecoration: 'none',
              fontSize: '18px',
              fontWeight: '500',
              marginBottom: '16px',
              border: 'none',
              cursor: 'pointer',
              transition: 'all 0.3s ease'
            }}
          >
            Login
          </a>

          <a
            href="/onboarding/conditions"
            style={{
              display: 'block',
              backgroundColor: 'white',
              color: '#2c3060',
              padding: '14px 24px',
              borderRadius: '12px',
              textDecoration: 'none',
              fontSize: '18px',
              fontWeight: '500',
              marginBottom: '24px',
              border: '2px solid #e4e0f0',
              cursor: 'pointer',
              transition: 'all 0.3s ease'
            }}
          >
            Get Started
          </a>

          <a
            href="https://immu.health/"
            target="_blank"
            rel="noopener noreferrer"
            style={{
              color: '#6e6d8f',
              textDecoration: 'none',
              fontSize: '14px'
            }}
          >
            More about Immu Health
          </a>
        </div>
      </div>
    </div>
  )
}
