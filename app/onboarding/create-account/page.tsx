...
export default function CreateAccountPage() {
  ...
  const handleSubmit = async () => {
    if (!agreedToTerms) {
      setError(t("onboarding.createAccount.termsError", "Please agree to the terms and conditions"))
      return
    }
    ...
    try {
      ...
      if (!user) {
        setError(t("onboarding.createAccount.sessionError", "Session not found. Please go back and sign up again."))
        setIsLoading(false)
        return
      }
  ...
  const titleKey = userName
    ? t("onboarding.createAccount.titleName", "Almost Done, {name}!").replace("{name}", userName)
    : t("onboarding.createAccount.title", "Almost Done!")
      ...
      <main className="flex-1 px-4 pb-8 overflow-auto">
        <div className="max-w-md mx-auto">
          <div className="mb-6 text-center">
            ...
            <p className="text-brand-dark/70">{t("onboarding.createAccount.subtitle", "Review and confirm to start your AIP journey.")}</p>
          </div>
          ...
          <div className="glass-card rounded-2xl p-6 space-y-4 mb-6">
            <div className="text-center mb-4">
              ...
              <h3 className="font-semibold text-lg">{t("onboarding.createAccount.profileReady", "Your profile is ready")}</h3>
              <p className="text-sm text-brand-dark/60 mt-2">
                {t("onboarding.createAccount.profileDesc", "We've collected all the information needed to personalize your AIP diet experience.")}
              </p>
              <div className="mt-4 p-3 bg-pink-50 border border-pink-200 rounded-lg">
                <p className="text-sm text-pink-700">
                  <span className="font-medium">{t("onboarding.createAccount.checkEmail", "Check your email!")}</span>{" "}
                  {t("onboarding.createAccount.emailDesc", "We've sent you a confirmation link to verify your account. Please confirm your email to sync your data across devices.")}
                </p>
              </div>
            </div>
            ...
            <div className="border-t border-brand-dark/10 pt-4">
              <div className="flex items-start">
                ...
                <label htmlFor="terms" className="ml-3 text-sm text-brand-dark">
                  {t("onboarding.createAccount.agreeTerms", "I agree to the")}{" "}
                  <a href="/terms" className="text-pink-500 hover:underline">
                    {t("onboarding.createAccount.termsLink", "terms and conditions")}
                  </a>{" "}
                  {t("onboarding.createAccount.and", "and")}{" "}
                  <a href="/privacy" className="text-pink-500 hover:underline">
                    {t("onboarding.createAccount.privacyLink", "privacy policy")}
                  </a>
                </label>
              </div>
            </div>
          ...
          >
            {isLoading
              ? t("onboarding.createAccount.starting", "Setting up...")
              : t("onboarding.createAccount.start", "Start My AIP Journey")}
          </button>
        </div>
      </main>
